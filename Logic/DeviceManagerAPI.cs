...
namespace DisableDevice
{
    public static class DeviceManagerApi
    {
        ...
        private const string GuidSHG8200 = "4d36e96c-e325-11ce-bfc1-08002be10318";
        private const string GuidSensor = "745a17a0-74d3-11d0-b6fe-00a0c90f57da";
        private const string SensorName = "HID-совместимый сенсорный экран";
        private static Logger logger = LogManager.GetCurrentClassLogger();
        private static string lastUsedInstancePath;
        public static bool IsDeviceExist(string guid, string partOfName)
        {
            ManagementObjectSearcher searcher =
                new ManagementObjectSearcher("root\\CIMV2", "SELECT * FROM Win32_PnPEntity"
                + $" WHERE ClassGuid = '{{{guid}}}'");
            var searchGet = searcher.Get();
            foreach (ManagementObject queryObj in searchGet)
            {

                var item = queryObj["Name"];
                if (item.ToString().Contains(partOfName))
                {
                    return true;
                }
            }
        ...
        public static int DeviceCount(string guid, string partOfName)
        {
            ManagementObjectSearcher searcher =
                new ManagementObjectSearcher("root\\CIMV2", "SELECT * FROM Win32_PnPEntity"
                + $" WHERE ClassGuid = '{{{guid}}}'");
        ...
        {
            return DeviceCount(GuidSensor, SensorName);
        }

        public static bool IsSensorExist(string partOfName)
        {
            return IsDeviceExist(GuidSensor, partOfName);
        }
        public static async Task<bool> IsSensorExist()
        {
        ...
        public static void ToogleSHG8200(bool isEnable)
        {
            //logger.Trace($"Start");
            Guid guidRedRat = new Guid(GuidSensor);//
            //string instancePath = @"USB\VID_112A&PID_0005\00003409";
            var instancePaths = GetInstancePaths(guidRedRat.ToString());
            foreach (var tab in instancePaths)
            {
